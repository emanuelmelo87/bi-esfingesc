...
| Movimentações | `/movimentacoes` | Em produção (novo) | O que mudou em cada carga: **Envio** (passou a constar como enviado, inclusive o que ainda não existia no banco), **Remoção** (estava enviado e deixou de estar, ex.: ratificou e depois removeu) e **Alteração** (continua enviado, mas mudou situação ou data). Mostra antes → depois, competência e horário. Filtros de município, fornecedor, movimento, campo e competência. Lista as 1000 mais recentes. Começa a ser preenchida a partir da primeira carga feita com a extensão atualizada. |
